...
const {
  ...
  delGroupChatbot,
} = require("../lib/MongoDB/MongoDb_Core");
let mergedCommands = [
  "admins",
  "admin",
  "setgcname",
    ...
    switch (inputCMD) {
      ...
      case "setgcname":
        if (!isAdmin) {
          await doReact("❌");
          return m.reply(
            `Kindly provide me Admin Role, Senpai❗, Grant me the power that I seek.`,
          );
        }
        if (!isBotAdmin) {
          await doReact("❌");
        ...
        try {
          ppgc = await shadow.profilePictureUrl(m.from, "image");
        } catch {
          ppgc = botImage1;
        }

        await shadow
          .groupUpdateSubject(m.from, text)
          .then((res) =>
            shadow.sendMessage(
              m.from,
              {
      ...
      case "del":
        if (!isAdmin) {
          await doReact("❌");
          return m.reply(
            `Senpai! ⚠️ This command is solely for the Admins! The power is not for everyone.`,
          );
        }
        if (!m.quoted) {
          await doReact("❔");
        ...
        if (!isBotAdmin) {
          if (!m.quoted.sender.includes(botNumber)) {
            await doReact("❌");
            return m.reply(
              `Gomen ne, Without *Admin* permission, I can only delete my own messages !`,
            );
          }
          key = {
            remoteJid: m.from,
          ...
          await doReact("📛");
          await shadow.sendMessage(m.from, { delete: key });
        } else {
          if (!isAdmin) {
            await doReact("❌");
            return m.reply(
              `Gomen ne, only *Admins* can delete other's messages !`,
            );
          }
          key = {
            remoteJid: m.from,
      ...
      case "demote":
        ...
        try {
          await shadow
            .groupParticipantsUpdate(m.from, [userId], "demote")
            .then((res) =>
              shadow.sendMessage(
                m.from,
                {
      ...
      case "grouplink":
        ...
        await doReact("🧩");
        let link = await shadow.groupInviteCode(m.from);
      ...
      case "gc":
        ...
        if (text === "close") {
          await shadow
            .groupSettingUpdate(m.from, "announcement")
            .then((res) => m.reply(`Group has been closed!`));
        } else if (text === "open") {
          await shadow
            .groupSettingUpdate(m.from, "not_announcement")
            .then((res) => m.reply(`Group has been opened!`));
        } else {
          await shadow.sendMessage(
            m.from,
            {
      ...
      case "leave":
        ...
        await shadow
          .sendMessage(m.from, {
            image: {
              url: "https://i.pinimg.com/originals/73/a4/25/73a425df6ca299f1cb907376f19d6cab.png",
            },
          ...
          .then(async () => {
            shadow.groupLeave(m.from).catch((e) => {
              shadow.sendMessage(
                m.from,
      ...
      case "promote":
        ...
        try {
          await shadow
            .groupParticipantsUpdate(m.from, [userId], "promote")
            .then((res) =>
              shadow.sendMessage(
                m.from,
                {
      ...
      case "remove":
        ...
        await shadow
          .groupParticipantsUpdate(m.from, [users], "remove")
          .then((res) =>
            shadow.sendMessage(
              m.from,
              {
      ...
      case "setppgc":
        ...
        let quotedimage = await shadow.downloadAndSaveMediaMessage(quoted);
      ...
      case "setgcdesc":
        ...
        await shadow.groupUpdateDescription(m.from, newGCdesc).then((res) =>
          shadow.sendMessage(
            m.from,
            {
              image: { url: ppgc, mimetype: "image/jpeg" },
      ...
      case "revoke":
        ...
        if (m.from == "120363040838753957@g.us") {
          await doReact("❌");
          return m.reply(
            "Sorry, this command is not allowed in *shadow Support Group* !\n\nYou are not allowed to change support group link !",
          );
        }
        await doReact("💫");

        await shadow
          .groupRevokeInvite(m.from)
          .then((res) =>
            shadow.sendMessage(
              m.from,
